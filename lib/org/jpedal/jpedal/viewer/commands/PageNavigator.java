...
public class PageNavigator {
     ...
     */
    private static JAITiffHelper tiffHelper;
					...
					if (updatedTotal <= commonValues.getPageCount()) {
						...
						if (commonValues.isMultiTiff()) {
							...
							if (count == 1 && decode_pdf.getDisplayView() == Display.FACING
								...
								corner.x = (int) ((((PdfDecoder)decode_pdf).getVisibleRect().getWidth() / 2) - pageW);
								corner.y = (int) (decode_pdf.getInsetH() + pageH);

								final Point cursor = new Point();
								cursor.x = (int) ((((PdfDecoder)decode_pdf).getVisibleRect().getWidth() / 2) + pageW);
    ...
    public static JAITiffHelper getTiffHelper() {
        return tiffHelper;
    }

    public static void setTiffHelper(final JAITiffHelper tiffHelp) {
        tiffHelper = tiffHelp;
    }
