...
public class JavaFXPreferences {
    ...
    private static ScrollPane getColorContent() {
        ...
        if(!invertHighlights.isEmpty() && invertHighlights.toLowerCase().equals("true")){
        ...
        if (!replaceTextCol.isEmpty() && replaceTextCol.toLowerCase().equals("true")) {
        ...
        if (!replaceDisplayCol.isEmpty() && replaceDisplayCol.toLowerCase().equals("true")) {
    ...
    private static ScrollPane getExtensionContent() {
        ...
        final Text freettsName = new Text("FreeTTS");
        final Text jaiName = new Text("JAI");
        ...
        final Text freettssDescr = new Text(Messages.getMessage("PdfExtensions.FreeTTS.text"));
        final Text jaiDescr = new Text(Messages.getMessage("PdfExtensions.JAI.text"));
        ...
        final Hyperlink freettsVersion;
        if (hasFreetts()) {
            version ="";// com.sun.speech.freetts.FreeTTS.VERSION.replaceAll("FreeTTS ", "");
            freettsVersion = new Hyperlink(version);
            details += "FreeTTS: ";// + com.sun.speech.freetts.FreeTTS.VERSION + '\n';
        } else {
            freettsVersion = new Hyperlink(Messages.getMessage("PdfExtensions.getText"));
            freettsVersion.setOnAction(new EventHandler<ActionEvent>() {
                @Override
                public void handle(final ActionEvent e) {
                    try {
                        BrowserLauncher.openURL(Messages.getMessage("PdfExtensions.FreeTTS.link"));
                    } catch (final Exception ex) {
                        ex.printStackTrace();
                    }
                }
        ...
        final Hyperlink jaiVersion;
        if (JAIHelper.isJAIOnClasspath()) {
            version = "Unknown Version";
            try {
                final Class jaic = Class.forName("javax.media.jai.JAI");
                final String className = jaic.getName().replace('.', '/');
                final String[] paths = jaic.getResource('/' + className + ".class").getPath().split("!");
                final URL file = new URL(paths[0]);
                final JarFile jar = new JarFile(file.getFile());
                if (!jar.getManifest().getMainAttributes().getValue("Implementation-Version").isEmpty()) {
                    version = jar.getManifest().getMainAttributes().getValue("Implementation-Version");
            ...
            final String build = JAI.getBuildVersion();
            details += "jai: " + version + ' ' + build + '\n';
            jaiVersion = new Hyperlink(version);
        } else {
            jaiVersion = new Hyperlink(Messages.getMessage("PdfExtensions.getText"));

            jaiVersion.setOnAction(new EventHandler<ActionEvent>() {
                @Override
                public void handle(final ActionEvent e) {
                    try {
                        BrowserLauncher.openURL(Messages.getMessage("PdfExtensions.JAI.link"));
                    } catch (final Exception ex) {
                        ex.printStackTrace();
                    }
                }
        ...
        contentGridPane.addColumn(0, nameTitle, bcmailName, cidName, freettsName, jaiName, javafxName, jceName, rhinoName);
        contentGridPane.addColumn(1, descriptionTitle, bcmailDescr, cidDescr, freettssDescr, jaiDescr, javafxDescr, jceDescr, rhinoDescr);
        contentGridPane.addColumn(2, versionTitle, bcmailVersion, cidVersion, freettsVersion, jaiVersion, javafxVersion, jceVersion, rhinoVersion);
    ...
    private static void loadSettings() {
        ...
        if (!propValue.isEmpty() && propValue.toLowerCase().equals("true")) {
        ...
        if(!changeTextAndLineart.isEmpty() && changeTextAndLineart.toLowerCase().equals("true")){
    ...
    private static boolean hasFreetts() {
        return JavaFXPreferences.class.getResourceAsStream("/com/sun/speech/freetts") != null;
    }
}
