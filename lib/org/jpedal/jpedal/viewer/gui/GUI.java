    ...
    public void init(final Commands currentCommands) {
        ...
        try {
            ...
            if (!propValue.isEmpty()) {
                ...
                //<start-fx>
                windowTitle = Messages.getMessage("PdfViewer.titlebar") + ' ' + PdfDecoderInt.version;
            ...
            if (!propValue.isEmpty()) {
                GUIDisplay.allowChangeCursor = propValue.toLowerCase().equals("true");
            }
...
//						resetRotationBox();
            ...
            if(DecoderOptions.showErrorMessages){
                ...
                if(status.contains("java.lang.OutOfMemoryError")){
                    ...
                    final Object[] options = {"Ok","Open website help page"};

                    final int n=currentGUI.showMessageDialog(status,options,0);
                    if(n==1){
                        try {
                            BrowserLauncher.openURL("http://www.idrsolutions.com/additional-jars/");
                        } catch (final Exception e1) {
                            if(LogWriter.isOutput()) { 
                                LogWriter.writeLog("Exception attempting to Launch Browser " + e1); 
                            } 
                        } catch (final Error err) {
                             if(LogWriter.isOutput()) { 
                                LogWriter.writeLog("Exception attempting to Launch Browser "  + err); 
                            }
                        }
                    }
                }
