...
//            pane.add(hTextButton, c);
        ...
        private JPanel createExtensionsPane() {
            ...
            c.gridy++;
            c.gridx=0;
            pane.add(new JLabel("FreeTTS"), c);
            c.gridx=1;
            pane.add(new JLabel("<html>"+Messages.getMessage("PdfExtensions.FreeTTS.text")), c);
            c.gridx=2;
            final JLabel freetts;
            if (hasFreetts()) {
                version = "";//com.sun.speech.freetts.FreeTTS.VERSION.replaceAll("FreeTTS ","");
                freetts = new JLabel("<html>"+version);
                details += "FreeTTS: ";//+com.sun.speech.freetts.FreeTTS.VERSION+ '\n';
            } else {
                freetts = new JLabel("<html><u>"+Messages.getMessage("PdfExtensions.getText")+"</u></html>");
                freetts.setForeground(Color.BLUE);
                freetts.addMouseListener(new Link(Messages.getMessage("PdfExtensions.FreeTTS.link")));
            }
            pane.add(freetts, c);
            ...
            c.gridy++;
            c.gridx=0;
            pane.add(new JLabel("JAI"), c);
            c.gridx=1;
            pane.add(new JLabel("<html>"+Messages.getMessage("PdfExtensions.JAI.text")), c);
            c.gridx=2;
            final JLabel jai;
            if (JAIHelper.isJAIOnClasspath()) {
                ...
                jai = new JLabel("<html>"+version);
            } else {
                jai = new JLabel("<html><u>"+Messages.getMessage("PdfExtensions.getText")+"</u></html>");
                jai.setForeground(Color.BLUE);
                jai.addMouseListener(new Link(Messages.getMessage("PdfExtensions.JAI.link")));
            }
            pane.add(jai, c);
