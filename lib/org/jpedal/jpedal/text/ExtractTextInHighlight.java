...
public class ExtractTextInHighlight {
     ...
     */
    public ExtractTextInHighlight(String file_name) {

//		get any user set dpi
        final String xmlFlag=System.getProperty("xml");
        if(xmlFlag!=null && xmlFlag.toLowerCase().equals("true")){
            ExtractTextInHighlight.useXMLExtraction=true;
        }
...
//			output_path.mkdirs();
    ...
    protected void decodeFile(final String file_name) {
         ...
         */
        outputDir = user_dir + "highlight" + separator + name + separator;
        ...
        if(!decodePdf.isExtractionAllowed()){
            ...
            try {
                for (int page = start; page < end + 1; page++) { //read pages
                    ...
                    final int[] currentRectangle = {x1,y2,x2-x1, y1-y2};

                    //decodePdf.updateCursorBoxOnScreen(currentRectangle, DecoderOptions.highlightColor);
                    decodePdf.getTextLines().clearHighlights();
                    decodePdf.getTextLines().addHighlights(new int[][]{currentRectangle}, false, page);
                    /**
                     *********************************************
                     */
                    final int[][] highlights = decodePdf.getTextLines().getHighlightedAreasAs2DArray(page);
                    if(highlights!=null){
                        for(int t=0; t!=highlights.length; t++){
                            String extractedText = "";

                            /**ensure co-ords in right order*/
                            int t_x1=highlights[t][0]-2;
                            int t_x2=highlights[t][0]+highlights[t][2]+4;
                            int t_y1=highlights[t][1]+highlights[t][3]+4;
                            int t_y2=highlights[t][1]-2;
